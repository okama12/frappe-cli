...
try:
    ...
    LOG_FILE = "frappe-installer.log" # Log in current directory if /var/log is not writable
...
class RichShell:
    ...
    def run(self, cmd, description, ignore_errors=False, input_text=None):
        ...
        try:
            ...
            if input_text:
                result = subprocess.run(cmd, input=input_text, check=True, capture_output=True, text=True)
            else:
...
def clean_nginx_upstreams(conf_path):
    ...
    try:
        ...
        while i < len(lines):
            ...
            if line.strip().startswith("upstream ") and "{" in line:
                ...
                if upstream_name in upstreams:
                    ...
                    console.print(f"[yellow]Removing duplicate upstream block '{upstream_name}' from {conf_path}[/yellow]")
                    logger.warning(f"[prod] Removing duplicate upstream block '{upstream_name}' from {conf_path}")
                    cleaned = True
        ...
        console.print(f"[bold red]✗ Failed to clean nginx config {conf_path}: {e}[/bold red]")
...
def prod(ctx, bench_name, dry_run, debug, ignore_errors):
    ...
    try:
        ...
        console.print("[bold red]Bench command not found in PATH. Please ensure bench is installed.[/bold red]")
    ...
    console.print("[blue]Performing comprehensive Nginx configuration cleanup...[/blue]")
    if not dry_run:
        ...
        shell_runner.run(["sudo", "rm", "-f", "/etc/nginx/sites-enabled/frappe-bench"], "Removing old default bench Nginx symlink", ignore_errors=True)
        shell_runner.run(["sudo", "rm", "-f", "/etc/nginx/sites-available/frappe-bench"], "Removing old default bench Nginx config file", ignore_errors=True)
        ...
        current_bench_nginx_symlink = os.path.join("/etc/nginx/sites-enabled", os.path.basename(bench_path))
        current_bench_nginx_config = os.path.join("/etc/nginx/sites-available", os.path.basename(bench_path))
        shell_runner.run(["sudo", "rm", "-f", current_bench_nginx_symlink], f"Removing old '{os.path.basename(bench_path)}' Nginx symlink", ignore_errors=True)
        shell_runner.run(["sudo", "rm", "-f", current_bench_nginx_config], f"Removing old '{os.path.basename(bench_path)}' Nginx config file", ignore_errors=True)
    ...
    if not dry_run:
        if not validate_nginx_config():
            console.print("[bold red]Nginx configuration is invalid even after cleanup. Please investigate manually.[/bold red]")
            if not ignore_errors:
                raise click.ClickException("Invalid nginx configuration after initial cleanup")

    # Run production setup with automatic confirmation
    console.print("[blue]Running bench setup production...[/blue]")
    production_cmd = [
        "sudo", bench_cmd, "setup", "production", os.getenv("USER")
    ]
    ...
    try:
        if not dry_run:
            ...
            if result.returncode != 0:
                console.print(f"[bold red]✗ Production setup failed:[/bold red]")
                ...
                if not dry_run:
                    shell_runner.run(["sudo", "rm", "-f", "/etc/nginx/sites-enabled/frappe-bench"], "Removing old default bench Nginx symlink (retry)", ignore_errors=True)
                    shell_runner.run(["sudo", "rm", "-f", "/etc/nginx/sites-available/frappe-bench"], "Removing old default bench Nginx config file (retry)", ignore_errors=True)
                    shell_runner.run(["sudo", "rm", "-f", current_bench_nginx_symlink], f"Removing old '{os.path.basename(bench_path)}' Nginx symlink (retry)", ignore_errors=True)
                    shell_runner.run(["sudo", "rm", "-f", current_bench_nginx_config], f"Removing old '{os.path.basename(bench_path)}' Nginx config file (retry)", ignore_errors=True)
                ...
                if validate_nginx_config():
                    ...
                    if result.returncode != 0:
                        if not ignore_errors:
                            ...
                            console.print("[bold red]✗ Production setup failed after retry. See details below:[/bold red]")
                            console.print(f"[red]STDOUT: {result.stdout}[/red]")
                            console.print(f"[red]STDERR: {result.stderr}[/red]")
                            raise click.ClickException(f"Production setup failed after retry: {result.stderr}")
                        else:
                            console.print("[yellow]Production setup failed after retry but continuing...[/yellow]")
                    else:
                        console.print("[green]✓ Production setup completed successfully on retry[/green]")
                else:
                    # --- BEGIN: Enhanced summary reporting and error context ---
                    if not ignore_errors:
                        console.print("[bold red]✗ Production setup failed and nginx config is invalid after retry cleanup. See details below:[/bold red]")
                        console.print(f"[red]STDOUT: {result.stdout}[/red]")
                        console.print(f"[red]STDERR: {result.stderr}[/red]")
                        raise click.ClickException("Production setup failed and nginx config is invalid after retry cleanup")
                    else:
                        console.print("[yellow]Production setup failed but continuing...[/yellow]")
            else:
                console.print("[green]✓ Production setup completed successfully[/green]")
        else:
            console.print(f"[yellow][dry-run] Would run: {' '.join(production_cmd)}[/yellow]")
    ...
    if not dry_run:
        console.print("[blue]Final Nginx configuration cleanup and service restart...[/blue]")
        # Perform comprehensive cleanup one last time
        shell_runner.run(["sudo", "rm", "-f", "/etc/nginx/sites-enabled/frappe-bench"], "Removing old default bench Nginx symlink (final)", ignore_errors=True)
        shell_runner.run(["sudo", "rm", "-f", "/etc/nginx/sites-available/frappe-bench"], "Removing old default bench Nginx config file (final)", ignore_errors=True)
        shell_runner.run(["sudo", "rm", "-f", current_bench_nginx_symlink], f"Removing old '{os.path.basename(bench_path)}' Nginx symlink (final)", ignore_errors=True)
        shell_runner.run(["sudo", "rm", "-f", current_bench_nginx_config], f"Removing old '{os.path.basename(bench_path)}' Nginx config file (final)", ignore_errors=True)
        clean_all_nginx_configs()
            ...
            for filename in os.listdir(nginx_sites_available):
                ...
                try:
                    ...
                    if re.search(r'server_name\s+[^;]+;', conf):
                        if len(os.listdir(nginx_sites_available)) == 1 or os.path.basename(bench_path) in conf or 'frappe' in conf:
                            new_conf, count = re.subn(r'server_name\s+[^;]+;', f'server_name {public_ip};', conf)
                            if count > 0:
                                ...
                                console.print(f"[green]✓ Updated server_name to {public_ip} in {file_path}[/green]")
                                logger.info(f"[prod] Updated server_name to {public_ip} in {file_path}")
                                updated = True
                                break
                except Exception as e:
                    console.print(f"[bold red]✗ Failed to update server_name in {file_path}: {e}[/bold red]")
                    logger.error(f"[prod] Failed to update server_name in {file_path}: {e}")
            if not updated:
                console.print(f"[yellow]No suitable NGINX config found to update server_name in {nginx_sites_available}[/yellow]")
        elif not public_ip:
            console.print("[yellow]Could not determine public IP. Skipping server_name update.[/yellow]")
        else:
            console.print(f"[yellow]NGINX config directory {nginx_sites_available} not found. Skipping server_name update.[/yellow]")
        ...
        if validate_nginx_config():
            ...
            ], "Restart supervisor", ignore_errors=ignore_errors)

            shell_runner.run([
                "sudo", "systemctl", "reload", "nginx"
            ], "Reload nginx", ignore_errors=ignore_errors)

            shell_runner.run([
                "sudo", "systemctl", "enable", "supervisor", "nginx"
            ], "Enable supervisor and nginx on boot", ignore_errors=ignore_errors)
        else:
            console.print("[bold red]✗ Final Nginx configuration is still invalid! Manual intervention required.[/bold red]")
            if not ignore_errors:
                raise click.ClickException("Final Nginx configuration validation failed")

    logger.info(f"[prod] Production environment configured for bench: {bench_name}")
    console.print(f"[bold green]✓ Production environment configured for bench: {bench_name}[/bold green]")
