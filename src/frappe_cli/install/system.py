...
def system(ctx, dry_run, debug, ignore_errors):
    ...
    if pkg_mgr == "apt":
        ...
        shell_runner.run(["sudo", "DEBIAN_FRONTEND=noninteractive", "apt", "upgrade", "-y"], "Upgrading packages", ignore_errors=ignore_errors)
        shell_runner.run([
            "sudo", "apt", "install", "-y",
            "curl", "wget", "git",
            "software-properties-common",
            "apt-transport-https", "ca-certificates"
        ], "Installing essential packages", ignore_errors=ignore_errors)
    elif pkg_mgr in ["dnf", "yum"]:
        shell_runner.run(["sudo", pkg_mgr, "makecache"], "Updating package lists", ignore_errors=ignore_errors)
        shell_runner.run(["sudo", pkg_mgr, "upgrade", "-y"], "Upgrading packages", ignore_errors=ignore_errors)
        shell_runner.run([
