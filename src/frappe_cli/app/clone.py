...
def fix_ownership(path):
    try:
        ...
        console.print(f"[yellow]Warning: Could not fix ownership for '{path}': {e}[/yellow]")
...
def clone(bench_name, repo_url, branch, dry_run, debug, ignore_errors):
    ...
    if not os.access(bench_path, os.W_OK):
        console.print(f"[yellow]Warning: Bench directory '{bench_path}' is not writable by user '{getpass.getuser()}'. You may encounter permission errors.[/yellow]")
    # --- New: Check assets and sites/assets permissions/ownership ---
    assets_dirs = [os.path.join(bench_path, 'assets'), os.path.join(bench_path, 'sites', 'assets')]
    for adir in assets_dirs:
        if os.path.exists(adir):
            if not os.access(adir, os.W_OK):
                console.print(f"[yellow]Warning: Directory '{adir}' is not writable by user '{getpass.getuser()}'. You may encounter asset build errors.[/yellow]")
                console.print(f"[bold red]To fix this, run:[/bold red] [green]sudo chown -R $USER:$USER '{adir}' && chmod -R u+w '{adir}'[/green]")
                console.print(f"[bold yellow]Please fix the permissions and re-run this command. Aborting.[/bold yellow]")
                raise click.ClickException(f"Directory '{adir}' is not writable by user '{getpass.getuser()}'. Fix permissions and try again.")
                ...
                for root, dirs, files in os.walk(adir):
                    for name in dirs + files:
                        ...
                        try:
                            stat = os.stat(path)
                            if stat.st_uid == 0:
                                console.print(f"[yellow]Warning: '{path}' is owned by root. This may cause permission errors during build.[/yellow]")
                        except FileNotFoundError:
                            # File or directory was removed between os.walk and os.stat; skip
                            continue
            except Exception as e:
                console.print(f"[yellow]Warning: Could not check ownership in '{adir}': {e}[/yellow]")
    ...
    if os.path.isdir(apps_dir):
        for d in os.listdir(apps_dir):
            ...
            if os.path.isdir(d_path):
                ...
                if d == app_name or d == app_name.replace('-', '_') or d == app_name.replace('_', '-'):
    ...
    if clone_result != 0 and os.path.isdir(app_path):
        ...
        try:
            ...
            console.print(f"[yellow]Removed incomplete app directory '{app_path}' due to clone failure.[/yellow]")
            logger.info(f"[app] Removed incomplete app directory '{app_path}' due to clone failure.")
        except Exception as e:
    ...
    console.print(f"[bold green]✓ App {app_name} cloned and validated as Frappe-compatible.[/bold green]")