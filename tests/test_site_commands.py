...
def test_site_delete_help():
    ...
    assert result.exit_code == 0
    assert ('Delete a Frappe site' in result.output or
            'Delete site' in result.output)
...
def test_site_list_help():
    ...
    assert result.exit_code == 0
    assert ('List all Frappe sites' in result.output or
            'List sites' in result.output)
...
def test_site_backup_help():
    ...
    assert result.exit_code == 0
    assert ('Backup a Frappe site' in result.output or
            'Backup site' in result.output)
...
def test_site_restore_help():
    ...
    assert result.exit_code == 0
    assert ('Restore a Frappe site' in result.output or
            'Restore site' in result.output)
