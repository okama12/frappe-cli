...
def test_site_backup_help():
    ...
    assert result.exit_code == 0
    assert 'Backup a Frappe site' in result.output or 'Backup site' in result.output

def test_site_restore_help():
    runner = CliRunner()
    result = runner.invoke(cli.cli, ['site', 'restore', '--help'])
    assert result.exit_code == 0
    assert 'Restore a Frappe site' in result.output or 'Restore site' in result.output
